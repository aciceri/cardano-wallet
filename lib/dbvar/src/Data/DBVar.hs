...
module Data.DBVar (
    ...
    -- * Testing
    , embedStore', updateLoad, updateLoadForCase, updateLoadHandle
    ) where
    ...
    let load = loadS bstore >>= \case
        ...
        write a = do
            ...
            mask $ \restore -> do
                ...
                writeMachine mab
        update = updateLoad load $ \a da -> do
            readMachine >>= \case
...
-- | Help to handle the case where a value is not yet loaded.
updateLoad :: (MonadThrow m, Exception e)
    => m (Either e t) -- ^ how to load the value
    -> (t -> da -> m b) -- ^ whot to do with the value
    -> Maybe t -- ^ already loaded value
    -> da
    -> m b
updateLoad load update' Nothing da = do
    ea <- load
    case ea of
        Left e -> throwIO e
        Right x -> update' x da
updateLoad _load  update' (Just x) da = update' x da

-- | Help to handle the case where a value is not yet loaded and the delta
-- is pattern matched on the caller side.
updateLoadForCase :: (MonadThrow m, Exception e)
    => m (Either e t) -- ^ how to load the value
    -> Maybe t -- ^ already loaded value
    -> (t -> m b) -- ^ whot to do with the value
    -> m b
updateLoadForCase load  Nothing update' = do
    ea <- load
    case ea of
        Left e -> throwIO e
        Right x -> update' x
updateLoadForCase  _load  (Just x) update' = update' x

-- | Help to handle the case where a value is not yet loaded.
updateLoadHandle :: (Exception e, Monad m)
    => m (Either e t) -- ^ how to load the value
    -> (e -> m b) -- ^ what to do with the error loading the value
    -> (t -> da -> m b) -- ^ whot to do with the value
    -> Maybe t -- ^ already loaded value
    -> da -- ^ delta
    -> m b
updateLoadHandle load handle update' Nothing da = do
    ea <- load
    case ea of
        Left e -> handle e
        Right x -> update' x da
updateLoadHandle _load _  update' (Just x) da = update' x da
